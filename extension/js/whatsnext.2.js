...
class whatsnext{
  constructor(time){
    ...
    this.periodInfo = {}
    this.schedule_base = null
    var schedule_base_url = "js/schedule2018_19.json"
    if(window.hasOwnProperty("$")){
        if($.hasOwnProperty("ajax")){
            var thisRef = this
            $.getJSON(schedule_base_url, function(res){
                        console.log(res) //
                        thisRef.schedule_base = res
                        thisRef.scheduleToday = thisRef.schedule();
                      })
        }
    }/*else{
